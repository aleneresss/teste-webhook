...
app.post("/webhook", async (req, res) => {
  console.log("AUTORIZACAO acionado", req.body.id);

  let conversationId =
    req.body.conversation_id ||
    req.body.id ||
    (req.body.messages && req.body.messages[0]?.conversation_id);
  ...
  if (!conversationId) {
    return res
      .status(400)
      .json({ erro: "conversation_id não fornecido ou detectado." });
  }
...
app.post("/webhook-saldo", async (req, res) => {
  console.log("SALDO acionado", req.body.id);
